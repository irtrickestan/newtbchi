	...
	if data.ID == "UpdateNewMessage" then
		...
		if msg.content_.ID == "MessageText" then
			...
			if is_sami(msg) then
				...
				elseif text:match('^([Aa]nswer) "(.*)" (.*)') then
					local txt, answer = text:match('^[Aa]nswer "(.*)" (.*)')
					redis:hset("botBOT-IDanswers", txt, answer)
					redis:sadd("botBOT-IDanswerslist", txt)
					return send(msg.chat_id_, msg.id_, "<i>Ответ для | </i>" .. tostring(txt) .. "<i> |Установите для :</i>\n" .. tostring(answer))
				elseif text:match("^([Dd]el answer ) (.*)") then
					local matches = text:match("^[Dd]el answer (.*)")
					redis:hdel("botBOT-IDanswers", matches)
					redis:srem("botBOT-IDanswerslist", matches)
					return send(msg.chat_id_, msg.id_, "<i>Ответ для | </i>" .. tostring(matches) .. "<i> | Очистить список автоответчиков.</i>")
				elseif text:match("^([Aa]uto ask) (.*)$") then
					...
					elseif matches == "off" then
						...
						return send(msg.chat_id_, 0, "<iАвтоответчик tabchi отключен.</i>")
					end
				...
				elseif text:match("^(панель)$") then
					...
					local txtadd = redis:get("botBOT-IDaddmsgtext") or  "Addi [Tabchi Bang]"
 ...
 Tabchi Bang By : Samiyar-Bang-@NoFooZiTM]]
		...
		elseif (msg.content_.ID == "MessageContact" and redis:get("botBOT-IDsavecontacts")) then
			...
			if redis:get("botBOT-IDaddmsg") then
				local answer = redis:get("botBOT-IDaddmsgtext") or "Addi [Tabchi Bang]"
